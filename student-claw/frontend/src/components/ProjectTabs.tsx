...
type Tab = "overview" | "tasks" | "documents" | "members" | "context";
    ...
    <nav className="flex gap-1 text-sm">
      {TABS.map((t) =>
        t.key === active ? (
          <span
            key={t.key}
            className="rounded-lg bg-brand-500/10 px-3 py-1 font-medium text-brand-300"
          >
            {t.label}
          </span>
        ) : (
          <Link
            ...
            className="rounded-lg px-3 py-1 text-slate-400 hover:bg-slate-800"
          >
            {t.label}
          </Link>
        ),
